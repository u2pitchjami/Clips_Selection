...
source /home/pipo/bin/Clips_Selection/.config_update.cfg
if [ -d TEMP ]; then
    rm -r TEMP
fi
mkdir TEMP
...
beet ls -a | tr -s " " | cut -d "-" -f1 | sed '/^ $/d' | sed 's/^[ \t]*//' | sed '$d' | uniq > TEMP/BEETSARTISTS
#sed -i '/^ $/d' TEMP/BEETSARTISTS
#sed -i '$d' TEMP/BEETSARTISTS
beet ls -a | tr -s " " | cut -d "-" -f2 | sed '/^ $/d' | sed 's/^[ \t]*//' | sed '$d' | uniq > TEMP/BEETSALBUMS
#sed -i 's/^[ \t]*//' TEMP/BEETSALBUMS

#sed -i '$d' TEMP/BEETSALBUMS

BEETSNBARTISTS=$(cat TEMP/BEETSARTISTS | wc -l)
BEETSNBALBUMS=$(cat TEMP/BEETSALBUMS | wc -l)
cat "${ACHECKER}" | cut -d "/" -f 6 | cut -d "(" -f1 | sed 's/ *$//' | uniq | sed '/^ $/d' > TEMP/BEETSACHECKERARTISTS
cat "${ACHECKER}" | cut -d "/" -f 7 | cut -d "-" -f3 | sed 's/^ *$//' | sed 's/^[ \t]*//' | uniq | sed '/^ $/d' > TEMP/BEETSACHECKERALBUMS
BEETSNBACHECKERARTISTS=$(cat TEMP/BEETSACHECKERARTISTS | wc -l)
BEETSNBACHECKERALBUMS=$(cat TEMP/BEETSACHECKERALBUMS | wc -l)
...
#do
#LIGNE=$(cat TEMP/BEETSACHECKERALBUMS | head -$b | tail +$b)
...
#do
#LIGNE=$(cat TEMP/BEETSACHECKERARTISTS | head -$c | tail +$c)
...
shuf -n 50 TEMP/BEETSARTISTS >> TEMP/BEETSACHECKERARTISTS
shuf -n 100 TEMP/BEETSALBUMS >> TEMP/BEETSACHECKERALBUMS
BEETSNBACHECKERARTISTS=$(cat TEMP/BEETSACHECKERARTISTS | wc -l)
BEETSNBACHECKERALBUMS=$(cat TEMP/BEETSACHECKERALBUMS | wc -l)
